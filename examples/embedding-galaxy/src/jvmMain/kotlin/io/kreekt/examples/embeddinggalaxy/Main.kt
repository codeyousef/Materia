...
fun main(): Unit = runBlocking {
    ...
    val window = glfwCreateWindow(1600, 900, "KreeKt – Embedding Galaxy", NULL, NULL)
    ...
    var lastWidth = 0
    var lastHeight = 0
    val capturePath = "examples/_captures/embedding-galaxy.png"
    ...
    glfwSetKeyCallback(window) { _, key, _, action, _ ->
        if (action == GLFW_PRESS) {
            when (key) {
                ...
                GLFW_KEY_Q -> scene.triggerQuery()
                GLFW_KEY_R -> scene.resetSequence()
                GLFW_KEY_P -> scene.togglePause()
            }
        }
    }
        ...
        if (!screenshotCaptured && lastWidth > 0 && lastHeight > 0) {
            if (captureCountdown > 0) {
                ...
                if (WindowCapture.capture(window, lastWidth, lastHeight, capturePath, renderer)) {
                    println("[EmbeddingGalaxy] Screenshot saved to $capturePath")
                }
