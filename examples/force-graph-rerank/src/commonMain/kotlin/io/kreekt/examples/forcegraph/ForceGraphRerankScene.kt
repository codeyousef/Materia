...
class ForceGraphRerankScene(
    ...
    private val material = PointsMaterial(color = Color(0xffffff), size = 1.5f, sizeAttenuation = true, vertexColors = true)
    ...
    private fun applyLayout(layout: Map<String, Vector3>) {
        ...
        for (i in 0 until config.nodeCount) {
            val idx = i * 3
            nodeBatch.setInstancePosition(i, currentPositions[idx], currentPositions[idx + 1], currentPositions[idx + 2])
        }
    ...
    private fun initialiseLines(edges: List<GraphEdge>) {
        ...
        for (edgeIndex in edges.indices) {
            ...
            lineBatch.setSegment(edgeIndex, 0f, 0f, 0f, 0f, 0f, 0f, Colors.azure.r, Colors.azure.g, Colors.azure.b, 0.2f)
        }
    }
    ...
    private fun bakeLayouts(graph: GraphData): LayoutComputation {
        ...
        val semanticResult = ForceLayout(seed = config.seed).bake(semanticGraph, config.layoutIterations, config.layoutTimeStep)
