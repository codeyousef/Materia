...
object ForceGraphLayoutGenerator {
    fun generate(config: ForceGraphScene.Config): ForceGraphLayout {
        ...
        repeat(nodeCount) { index ->
            ...
            val baseAngle =
                ((cluster.toFloat() + random.nextFloat() * 0.35f) / clusterCount.toFloat()) * (PI.toFloat() * 2f)
            ...
            semanticPositions[tfidfBase + 2] =
                sin(baseAngle + random.gaussian(0.25f)) * semanticRadius
        ...
        repeat(edgeCount) { edgeIndex ->
            ...
            semanticEdgeStrength[edgeIndex] =
                0.5f + random.nextFloat() * 0.5f * (1f - clusterSim * 0.4f)
        }
