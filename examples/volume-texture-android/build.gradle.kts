...
android {
    namespace = "io.materia.examples.volumetexture.android"
    compileSdk = libs.versions.androidCompileSdk.get().toInt()

    defaultConfig {
        applicationId = "io.materia.examples.volumetexture.android"
...
dependencies {
    implementation(project(":examples:volume-texture"))
...
fun runCommand(vararg args: String, printOutput: Boolean = true): Pair<Int, List<String>> {
    return try {
        ...
        if (printOutput) {
            println("Failed to run ${args.joinToString(" ")}: ${error.message}")
        }
...
tasks.register("runAndroid") {
    group = "run"
    description = "Install the Volume Texture Android demo on a connected device or emulator"
    ...
    doLast {
        val component = "io.materia.examples.volumetexture.android/.VolumeTextureActivity"
        ...
        if (adb == null) {
            println("adb was not found. Set ADB, ANDROID_HOME, or ANDROID_SDK_ROOT, or install platform-tools.")
            return@doLast
        }
        ...
        if (!deviceAvailable) {
            println("No Android device detected. Install manually with:")
        ...
        if (runCommand(adb.absolutePath, "install", "-r", apk.absolutePath).first != 0) {
            println("Failed to install APK automatically. Try:")
            println("    ${adb.absolutePath} install -r ${apk.absolutePath}")
        }

        if (runCommand(adb.absolutePath, "shell", "am", "start", "-n", component).first != 0) {
            println("Unable to launch automatically. Start manually with:")
            println("    ${adb.absolutePath} shell am start -n $component")
        }
    }
}
...
tasks.register("smokeAndroid") {
    group = "verification"
    description = "Install, launch, and verify the Volume Texture Android demo boot log on a connected device or emulator"
    dependsOn("assembleDebug")
    ...
    doLast {
        ...
        val component = "io.materia.examples.volumetexture.android/.VolumeTextureActivity"
        val packageName = "io.materia.examples.volumetexture.android"
        val apk = layout.buildDirectory
            .file("outputs/apk/debug/${project.name}-debug.apk")
            .get()
            .asFile
        ...
        check(runCommand(adb.absolutePath, "shell", "am", "start", "-n", component).first == 0) {
            "Failed to launch VolumeTextureActivity for smoke test."
        }
        ...
        while (System.currentTimeMillis() < deadline) {
            ...
            if (output.any { line -> line.contains(successMarker) }) {
                println("Volume Texture Android smoke test passed")
                return@doLast
            }

            val failure = output.firstOrNull { line -> failureMarkers.any(line::contains) }
            if (failure != null) {
                error("Volume Texture Android smoke test failed: $failure")
            }
        }

        val tail = lastOutput.takeLast(20).joinToString("\n")
        error("Timed out waiting for VolumeTextureActivity boot log. Last log lines:\n$tail")
    }
}