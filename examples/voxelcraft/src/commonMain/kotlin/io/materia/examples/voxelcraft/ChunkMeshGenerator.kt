 ...
 *
 * Output: Materia BufferGeometry with position, normal, uv, and color attributes.
 */
object ChunkMeshGenerator {
    ...
    suspend fun generate(chunk: Chunk): BufferGeometry {
        ...
        val normals = ArrayList<Float>(estimatedVertices)
        val uvs =
            ArrayList<Float>(estimatedVertices * 2 / 3) // UVs are 2D (2 components per vertex)
        val colors = ArrayList<Float>(estimatedVertices)
        ...
        for (w in 0..wMax) {
            ...
            for (u in 0..uMax) {
                for (v in 0..vMax) {
                    val pos = DirectionHelper.getPosition(direction, u, v, w)
                    val neighborPos =
                        DirectionHelper.getPosition(direction, u, v, w + neighborOffset)

                    val block = chunk.getBlockSafe(pos[0], pos[1], pos[2])
                    val neighbor =
                        chunk.getNeighborBlock(neighborPos[0], neighborPos[1], neighborPos[2])
