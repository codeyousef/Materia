    ...
    fun `criteria extractor covers all result types and fallback`() {
        val compilation = CompilationResult(
            ...
            platformResults = mapOf(
                "jvm" to PlatformCompilationStatus(platform = "jvm", success = false, errorMessages = listOf("Missing symbol")),
                "js" to PlatformCompilationStatus(platform = "js", success = true)
            ),
            errors = listOf(CompilationError(file = "Main.kt", line = 42, column = 5, message = "Unresolved reference"))
        )
        ...
        val constitutional = ConstitutionalCompliance(
            ...
            tddCompliance = TddComplianceResult(isCompliant = false, testsWrittenFirst = true, testCoveragePercent = 70f, untestableFunctions = listOf("Renderer.render")),
            ...
            codeSmells = listOf(CodeSmell(type = "LongMethod", file = "Renderer.kt", line = 120, description = "Method too long", severity = "MEDIUM"))
        )
        ...
        val compilationCriteria = criteriaExtractor.extractCriteria(compilation, "CompilationValidator")
        val testCriteria = criteriaExtractor.extractCriteria(tests, "TestCoverageValidator")
        val performanceCriteria = criteriaExtractor.extractCriteria(performance, "PerformanceValidator")
        val constitutionalCriteria = criteriaExtractor.extractCriteria(constitutional, "ConstitutionalValidator")
            ...
            results = listOf(
                ValidatorResult(
                    ...
                    result = CompilationResult(
                        ...
                        platformResults = mapOf(
                            "jvm" to PlatformCompilationStatus("jvm", success = false, errorMessages = listOf("Missing symbol")),
                            "js" to PlatformCompilationStatus("js", success = true)
                        ),
                        errors = listOf(CompilationError(file = "Main.kt", line = 12, column = 4, message = "Unresolved reference"))
                    ),
                    error = null
                ),
                ValidatorResult(
