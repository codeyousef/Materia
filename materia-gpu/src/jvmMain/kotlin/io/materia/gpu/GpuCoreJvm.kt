...
import io.materia.renderer.gpu.GpuBuffer as RendererGpuBuffer
import io.materia.renderer.gpu.GpuContext as RendererGpuContext
import io.materia.renderer.gpu.GpuDevice as RendererGpuDevice
import io.materia.renderer.gpu.GpuDeviceFactory as RendererGpuDeviceFactory
import io.materia.renderer.gpu.GpuQueue as RendererGpuQueue
import io.materia.renderer.gpu.GpuTexture as RendererGpuTexture
import io.materia.renderer.gpu.GpuTextureView as RendererGpuTextureView
import io.materia.renderer.gpu.GpuSampler as RendererGpuSampler
...
import io.materia.renderer.vulkan.VulkanSwapchain
...
actual class GpuDevice actual constructor(
    ...
    internal fun mapAndCopy(buffer: RendererGpuBuffer, data: ByteArray, byteOffset: Long) {
        ...
        MemoryStack.stackPush().use { stack ->
            ...
            val result =
                vkMapMemory(handle, buffer.memory, byteOffset, data.size.toLong(), 0, pointerBuffer)
    ...
    internal fun obtainRenderPass(key: RenderPassKey): Long {
        ...
        MemoryStack.stackPush().use { stack ->
            val attachmentDescriptions =
                VkAttachmentDescription.calloc(key.colorAttachments.size, stack)
...
actual class GpuSurface actual constructor(
    ...
    actual fun resize(width: Int, height: Int) {
        ...
        if (swapchainManager != null) {
            ...
            preferredFormat = actualFormat
            configuration =
                configuration?.copy(width = width, height = height, format = actualFormat)
        } else {
...
actual class GpuBindGroup actual constructor(
    ...
    private fun allocateDescriptorSet() {
        ...
        MemoryStack.stackPush().use { stack ->
            ...
            descriptor.entries.forEachIndexed { index, entry ->
                val layoutEntry =
                    layout.descriptor.entries.firstOrNull { it.binding == entry.binding }
...
actual class GpuRenderPipeline actual constructor(
    ...
    init {
        ...
        MemoryStack.stackPush().use { stack ->
            ...
            val layoutResult =
                vkCreatePipelineLayout(deviceHandle, pipelineLayoutInfo, null, pPipelineLayout)
            ...
            val attributeDescriptions = if (vertexLayouts.isNotEmpty()) {
                ...
                if (totalAttributes > 0) {
                    VkVertexInputAttributeDescription.calloc(totalAttributes, stack)
                        .also { attributes ->
            ...
            val pipelineResult = vkCreateGraphicsPipelines(
                deviceHandle,
                VK_NULL_HANDLE,
                pipelineInfo,
                null,
...
actual class GpuRenderPassEncoder actual constructor(
    ...
    init {
        ...
        MemoryStack.stackPush().use { stack ->
            ...
            val framebufferResult =
                vkCreateFramebuffer(deviceHandle, framebufferInfo, null, pFramebuffer)
...
private fun GpuBlendMode.applyTo(attachment: VkPipelineColorBlendAttachmentState): VkPipelineColorBlendAttachmentState =
    when (this) {
...
private fun GpuTextureUsageFlags.toVulkanImageUsage(): Int {
    var usage = 0
    if (this and GpuTextureUsage.COPY_SRC.mask != 0) usage =
        usage or VK_IMAGE_USAGE_TRANSFER_SRC_BIT
    if (this and GpuTextureUsage.COPY_DST.mask != 0) usage =
        usage or VK_IMAGE_USAGE_TRANSFER_DST_BIT
    if (this and GpuTextureUsage.TEXTURE_BINDING.mask != 0) usage =
        usage or VK_IMAGE_USAGE_SAMPLED_BIT
    if (this and GpuTextureUsage.STORAGE_BINDING.mask != 0) usage =
        usage or VK_IMAGE_USAGE_STORAGE_BIT
    if (this and GpuTextureUsage.RENDER_ATTACHMENT.mask != 0) usage =
        usage or VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT
    return usage
}
