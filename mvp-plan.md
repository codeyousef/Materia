...
> Running any one should put “first pixels” on screen.
...
**If any of these do not show “first pixels” in <5 minutes, fix before adding features.**

---

## 🧭 Target Matrix (one API, per‑target backends)

| Target                              | Backend  | Shader Path         | Surface                      |
|-------------------------------------|----------|---------------------|------------------------------|
| wasmJs (Browser)                    | WebGPU   | WGSL                | HTMLCanvas + `navigator.gpu` |
| jvm / linuxX64 / mingwX64 (Desktop) | Vulkan   | WGSL → SPIR‑V       | GLFW + Vulkan swapchain      |
| macOS / iOS                         | MoltenVK | WGSL → SPIR‑V → MVK | CAMetalLayer via MoltenVK    |
| Android                             | Vulkan   | WGSL → SPIR‑V       | SurfaceView + Vulkan         |
...
## 📐 API & DX Principles (must hold for every public API)

* **Kotlin‑first**: idiomatic naming, default params, named args, extension functions, KDoc.
* **Zero‑cost**: `@JvmInline` value classes, `inline` funcs, reified generics, no hidden allocs in hot paths.
* **KMP‑first**: one public API via `expect/actual`.
...
* **Structured concurrency**: coroutines for IO/asset upload only; never inside per‑frame loops.
* **Escape hatches**: low‑level buffer/descriptor access for perf work.

**Kotlin features to use deliberately**

* Context receivers for render‑scoped ops:
...
* Persistent mapping (Vulkan), ring buffers for per‑frame uniforms; bind‑group/descriptor caches.
...
* [ ] `Vec2/3/4`, `Mat4`, `Quat`, `Color` — value classes, inline ops
...
* Editor/inspector UI, universal hot‑reload

---

## 🎯 Adoption Gates (ship only when all pass)
...
* [ ] **Force Graph** toggle is visually obvious & stutter‑free.
* [ ] APIs feel **Kotlin‑native** (named args, defaults, extensions, KDoc).
* [ ] Users can change one number (e.g., `count = 20000 → 8000`) and instantly see the effect.

---

## 🧩 Hello Triangle (reference snippet)
...
## 🔬 Performance Notes (TL;DR)

* Keep **uniform buffers** in ring buffers; avoid per‑frame mallocs.
...
*If a checklist item is ambiguous, prioritize: time‑to‑first‑pixels, smoothness, and Kotlin DX.*
