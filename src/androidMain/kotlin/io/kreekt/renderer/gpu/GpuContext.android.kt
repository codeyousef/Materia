...
private object VulkanBridgeProxy {
    private val clazz = runCatching { Class.forName("io.kreekt.gpu.bridge.VulkanBridge") }
        .getOrElse { cause ->
            throw IllegalStateException(
                "Android Vulkan bridge not available. Ensure :kreekt-gpu module is on the classpath.",
                cause
            )
        }
...
private fun String.toTextureFormatInfo(): TextureFormatInfo = when (normaliseFormat()) {
    ...
    "depth24plus", "depth24plusstencil8", "depth24plusstencil" -> TextureFormatInfo(nativeFormat = 3, isDepth = true)
...
private fun Int.toNativeBufferUsage(): Int {
    ...
    if (hasBufferUsage(GpuBufferUsage.INDIRECT)) flags = flags or VK_BUFFER_USAGE_INDIRECT_BUFFER_BIT
    return flags
}
...
private fun GpuTextureDescriptor.nativeUsageFlags(formatInfo: TextureFormatInfo): Int {
    var flags = 0
    if (usage.hasTextureUsage(GpuTextureUsage.COPY_SRC)) flags = flags or VK_IMAGE_USAGE_TRANSFER_SRC_BIT
    if (usage.hasTextureUsage(GpuTextureUsage.COPY_DST)) flags = flags or VK_IMAGE_USAGE_TRANSFER_DST_BIT
    if (usage.hasTextureUsage(GpuTextureUsage.TEXTURE_BINDING)) flags = flags or VK_IMAGE_USAGE_SAMPLED_BIT
    if (usage.hasTextureUsage(GpuTextureUsage.STORAGE_BINDING)) flags = flags or VK_IMAGE_USAGE_STORAGE_BIT
...
actual class GpuCommandEncoder internal constructor(
    ...
    actual fun finish(): GpuCommandBuffer {
        ...
        VulkanBridgeProxy.vkDestroyCommandEncoder(device.instanceHandle, device.deviceHandle, handle)
        finished = true
        handle = 0L
...
actual class GpuTexture internal constructor(
    ...
    actual fun createView(descriptor: GpuTextureViewDescriptor?): GpuTextureView {
        ...
        val overrideFormat = descriptor?.format?.let { it.toTextureFormatInfo().nativeFormat } ?: VK_FORMAT_UNDEFINED
