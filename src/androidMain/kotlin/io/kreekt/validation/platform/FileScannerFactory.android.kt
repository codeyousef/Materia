...
private class AndroidFileScanner : FileScanner {
    ...
    override suspend fun findFiles(baseDir: String, patterns: List<String>): List<FileInfo> = withContext(dispatcher) {
        ...
        Files.walk(dir).use { paths ->
            paths.filter { Files.isRegularFile(it) }.forEach { path ->
                if (patterns.isEmpty() || patterns.any { pattern -> path.fileName.toString().matchesGlob(pattern) }) {
    ...
    override suspend fun writeFileContent(path: String, content: String): Boolean = withContext(dispatcher) {
        try {
            val file = Paths.get(path)
    ...
    override suspend fun copyFile(sourcePath: String, destinationPath: String): Boolean = withContext(dispatcher) {
        try {
    ...
    override suspend fun moveFile(sourcePath: String, destinationPath: String): Boolean = withContext(dispatcher) {
        try {
            val source = Paths.get(sourcePath)
            val destination = Paths.get(destinationPath)
            Files.createDirectories(destination.parent)
