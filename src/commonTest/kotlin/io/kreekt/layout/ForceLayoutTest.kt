...
class ForceLayoutTest {
    ...
    fun layoutKeepsNodesWithinRadius() {
        ...
        val result = ForceLayout(seed = 7L, boundingRadius = 50f).bake(graph, iterations = 100, timeStep = 0.02f)
    ...
    private fun sampleGraph(nodeCount: Int): GraphData {
        ...
        val edges = buildList {
            for (i in 0 until nodeCount - 1) {
                add(GraphEdge(source = "n$i", target = "n${i + 1}", weight = 1f, type = EdgeType.UNDIRECTED))
            }
            add(GraphEdge(source = "n0", target = "n${nodeCount - 1}", weight = 0.5f, type = EdgeType.UNDIRECTED))
        }
