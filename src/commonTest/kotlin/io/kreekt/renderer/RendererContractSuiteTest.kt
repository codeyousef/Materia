...
private class SimulatedRenderer(
    ...
    override suspend fun initialize(config: RendererConfig): io.kreekt.core.Result<Unit> {
        return io.kreekt.core.Result.Success(Unit)
    }

    override fun render(scene: Scene, camera: io.kreekt.camera.Camera) {
...
private object SimulatedRendererFactory {
    ...
    fun create(
        ...
        return io.kreekt.core.Result.Success(SimulatedRenderer(selected.type, selected.capabilities))
    }
}
...
class RendererContractSuiteTest {
    ...
    fun rendererLifecycleUpdatesState() {
        val renderer = SimulatedRendererFactory.create(availableBackends).getOrThrow() as SimulatedRenderer
    ...
    fun rendererFactoryFailsWhenNoBackendAvailable() {
        ...
        assertTrue(result is io.kreekt.core.Result.Error)
        val exception = (result as io.kreekt.core.Result.Error).exception
