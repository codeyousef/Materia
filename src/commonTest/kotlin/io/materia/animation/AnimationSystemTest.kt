...
import io.materia.animation.skeleton.IKSolverType
...
class AnimationSystemTest {
    ...
    fun testMorphTargetAnimator() {
        ...
        assertTrue(
            mesh.morphTargetInfluences.any { it > 0f },
            "Some morph targets should be influenced"
        )
    }
    ...
    fun testAnimationCompression() {
        ...
        assertTrue(
            compressedClip.tracks.size == originalClip.tracks.size,
            "Track count should be preserved"
        )
...
private sealed class AnimationException(message: String, cause: Throwable? = null) :
    Exception(message, cause) {
    class InvalidParameters(message: String) : AnimationException(message)
}

private sealed class IKException(message: String, cause: Throwable? = null) :
    Exception(message, cause)

private sealed class MorphException(message: String, cause: Throwable? = null) :
    Exception(message, cause)
