...
class LayersContractTest {
    ...
    fun testFilterDuringRaycasting() {
        ...
        for (obj in objects) {
            // Convert Object3D.layers mask to io.materia.layers.Layers for testing
            val objLayers = io.materia.layers.Layers()
...
// Supporting classes - using real Layers, Object3D, Camera from io.materia.layers
// All classes already have layers property defined except Raycaster

// Extension for Raycaster (until it's added to main class)
private val raycasterLayersMap = mutableMapOf<Raycaster, io.materia.layers.Layers>()
val Raycaster.layers: io.materia.layers.Layers
    get() = raycasterLayersMap.getOrPut(this) { io.materia.layers.Layers() }