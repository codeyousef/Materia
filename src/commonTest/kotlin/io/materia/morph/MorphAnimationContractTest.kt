...
class MorphAnimationContractTest {
    ...
    fun testBlendMultipleAnimations() {
        ...
        assertTrue(
            smileAction.weight < 0.7f,
            "Smile weight should be < 0.7, got ${smileAction.weight}"
        )
        assertTrue(
            frownAction.weight > 0.3f,
            "Frown weight should be > 0.3, got ${frownAction.weight}"
        )
        ...
        assertTrue(
            smileAction.weight < 0.5f,
            "Smile weight should continue fading, got ${smileAction.weight}"
        )
        ...
        mixer.update(0.5f)
        assertTrue(
            smileAction.weight < 0.01f,
            "Smile should be fully faded out, got ${smileAction.weight}"
        )
    }
        ...
        for (i in 0 until 10) {
            ...
            clips.add(
                AnimationClip(
                    "clip_$i",
                    2.0f,
                    tracks.map { it.keyframeTrack as KeyframeTrack })
            )
        }
...
class MorphAnimationMixer(private val geometry: BufferGeometry) {
    ...
    private val morphTargetCount =
        (0 until 8).count { geometry.getAttribute("morphTarget$it") != null }
    ...
    fun update(deltaTime: Float) {
        ...
        if (fadeDirection != 0 && fadeDuration > 0) {
            ...
            if (fadeProgress >= 1f) {
                ...
                weight =
                    fadeStartWeight + (fadeTargetWeight - fadeStartWeight) * fadeProgress.coerceIn(
                        0f,
                        1f
                    )
                isFading = true
