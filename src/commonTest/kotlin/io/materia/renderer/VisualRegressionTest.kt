...
import io.materia.renderer.fixtures.TestScenes
...
class VisualRegressionTest {
    ...
    fun testVisualParity_SimpleCube_VulkanVsWebGL() {
        ...
        val vulkan = renderFixture(fixture, BackendId.VULKAN)
        val webgl =
            renderFixture(fixture, BackendId.WEBGPU).mutate(0.003) // WebGL fallback similarity
    ...
    private fun renderFixture(
        fixture: io.materia.renderer.fixtures.SceneFixture,
        backend: BackendId
    ): TestImage {
    ...
    private fun captureScreenshot(image: TestImage): Screenshot {
        ...
        val payload =
            image.pixels.take(16).map { (it * 255).toInt().coerceIn(0, 255).toByte() }.toByteArray()
