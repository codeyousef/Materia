...
class BackendNegotiationContractTest {
    ...
    fun testHappyPath_WebGPU_InitializesSuccessfully() = runTest {
        ...
        val selection =
            negotiator.selectBackend(negotiator.detectCapabilities(CapabilityRequest()), profiles())
        val handle =
            negotiator.initializeBackend(selection, SurfaceConfig(width = 1280, height = 720))
    ...
    fun testHappyPath_Vulkan_InitializesSuccessfully() = runTest {
        ...
        val selection =
            negotiator.selectBackend(negotiator.detectCapabilities(CapabilityRequest()), profiles())
        val handle =
            negotiator.initializeBackend(selection, SurfaceConfig(width = 1920, height = 1080))
        ...
        override suspend fun selectBackend(
            ...
            val preferred = report.preferredBackend?.let { preferredId ->
                profiles.firstOrNull {
                    it.backendId == preferredId && validateParityRequirements(
                        it,
                        report
                    )
                }
