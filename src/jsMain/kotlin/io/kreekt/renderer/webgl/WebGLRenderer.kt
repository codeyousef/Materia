...
class WebGLRenderer(
    ...
    override suspend fun initialize(config: RendererConfig): io.kreekt.core.Result<Unit> {
        return try {
            ...
            resize(canvas.width.takeIf { it > 0 } ?: canvas.clientWidth, canvas.height.takeIf { it > 0 } ?: canvas.clientHeight)
            initialised = true
            io.kreekt.core.Result.Success(Unit)
        } catch (ex: RendererInitializationException) {
            io.kreekt.core.Result.Error(ex.message ?: "Failed to initialise WebGL renderer", ex)
        } catch (ex: Throwable) {
            ...
            io.kreekt.core.Result.Error(wrapped.message ?: "Failed to initialise WebGL renderer", wrapped)
        }
    }
    ...
    private fun compileShader(type: Int, source: String, name: String): org.khronos.webgl.WebGLShader {
    ...
    private fun queryCapabilities(): RendererCapabilities {
        val maxTextureSize = (gl.getParameter(WebGLRenderingContext.MAX_TEXTURE_SIZE) as? Int) ?: 2048
        val maxCubeMapSize = (gl.getParameter(WebGLRenderingContext.MAX_CUBE_MAP_TEXTURE_SIZE) as? Int) ?: maxTextureSize
        val maxVertexAttribs = (gl.getParameter(WebGLRenderingContext.MAX_VERTEX_ATTRIBS) as? Int) ?: 16
        val maxVertexUniforms = (gl.getParameter(WebGLRenderingContext.MAX_VERTEX_UNIFORM_VECTORS) as? Int) ?: 256
        val maxFragmentUniforms = (gl.getParameter(WebGLRenderingContext.MAX_FRAGMENT_UNIFORM_VECTORS) as? Int) ?: 256
        val maxVertexTextures = (gl.getParameter(WebGLRenderingContext.MAX_VERTEX_TEXTURE_IMAGE_UNITS) as? Int) ?: 0
        val maxFragmentTextures = (gl.getParameter(WebGLRenderingContext.MAX_TEXTURE_IMAGE_UNITS) as? Int) ?: 16
        val maxCombinedTextures = (gl.getParameter(WebGLRenderingContext.MAX_COMBINED_TEXTURE_IMAGE_UNITS) as? Int) ?: (maxFragmentTextures + maxVertexTextures)
        ...
        val shadingLanguage = gl.getParameter(WebGLRenderingContext.SHADING_LANGUAGE_VERSION) as? String ?: "GLSL"
        ...
        return RendererCapabilities(
            ...
            maxColorAttachments = if (isWebGL2Context) {
                runCatching { gl.getParameter(MAX_COLOR_ATTACHMENTS_CONST) as? Int }.getOrNull() ?: 1
            } else 1,
            maxSamples = maxSamples,
            maxAnisotropy = maxAnisotropy,
            maxUniformBufferSize = if (isWebGL2Context) {
                runCatching { gl.getParameter(MAX_UNIFORM_BLOCK_SIZE_CONST) as? Int }.getOrNull() ?: 16384
            } else 16384,
            maxUniformBufferBindings = if (isWebGL2Context) {
                runCatching { gl.getParameter(MAX_UNIFORM_BUFFER_BINDINGS_CONST) as? Int }.getOrNull() ?: 36
            } else 36,
            textureFormats = supportedTextureFormats,
            depthFormats = setOf(DepthFormat.DEPTH24_STENCIL8),
            extensions = gl.getSupportedExtensions()?.mapNotNull { it as? String }?.toSet() ?: emptySet(),
            vendor = vendor,
            renderer = renderer,
            version = version,
    ...
    private fun updateClearColor(scene: Scene) {
        ...
        val targetColor = when (background) {
            is Background.Color -> floatArrayOf(background.color.r, background.color.g, background.color.b, background.color.a)
    ...
    private fun updateStats(drawCalls: Int, triangles: Int, frameDurationMs: Double, timestamp: Double) {
        val fps = if (frameDurationMs > 0.0) 1000.0 / frameDurationMs else stats.fps
        val bufferMemory = meshBuffers.values.sumOf { it.vertexByteSize.toLong() + it.indexByteSize.toLong() }
