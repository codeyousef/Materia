...
import io.kreekt.renderer.webgpu.RenderStatsTracker
...
import io.kreekt.renderer.gpu.GpuTextureUsage
...
internal class WebGPUMaterialTextureManager(
    ...
    fun onDeviceReady(device: GpuDevice) {
        ...
        fallbackNormal = createFallbackTexture(device, byteArrayOf(127, 127, 255.toByte(), 255.toByte()))
    }
    ...
    fun prepare(
        ...
        if (useAlbedo) {
            val textureBinding = descriptor.bindingFor(MaterialBindingSource.ALBEDO_MAP, MaterialBindingType.TEXTURE_2D) ?: return null
            val samplerBinding = descriptor.bindingFor(MaterialBindingSource.ALBEDO_MAP, MaterialBindingType.SAMPLER) ?: return null
        ...
        if (useNormal) {
            val textureBinding = descriptor.bindingFor(MaterialBindingSource.NORMAL_MAP, MaterialBindingType.TEXTURE_2D) ?: return null
            val samplerBinding = descriptor.bindingFor(MaterialBindingSource.NORMAL_MAP, MaterialBindingType.SAMPLER) ?: return null
    ...
    private fun acquireTexture(device: GpuDevice, texture: Texture2D?): CachedTexture? {
        ...
        val view = gpuTexture.createView(GpuTextureViewDescriptor(dimension = GpuTextureViewDimension.D2))
    ...
    private fun createFallbackTexture(device: GpuDevice, data: ByteArray): CachedTexture? {
        ...
        return CachedTexture(gpuTexture, view, version = 0, width = 1, height = 1, trackedBytes = trackedBytes)
    }

    private fun writeTextureBytes(device: GpuDevice, texture: GpuTexture, width: Int, height: Int, data: ByteArray) {
