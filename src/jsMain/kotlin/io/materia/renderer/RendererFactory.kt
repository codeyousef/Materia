...
actual object RendererFactory {
    ...
    actual suspend fun create(
        ...
        if (preferWebGPU) {
            try {
                ...
                if (initResult is io.materia.core.Result.Success) {
                    console.log("[Materia] Selected backend: WebGPU")
                    return io.materia.core.Result.Success(renderer)
                } else if (initResult is io.materia.core.Result.Error) {
                    console.warn("[Materia] WebGPU initialization failed: ${initResult.message}")
                }
            } catch (e: Throwable) {
                console.warn("[Materia] WebGPU creation failed: ${e.message}, falling back to WebGL")
            }
        }
        ...
        if (BackendType.WEBGL in availableBackends) {
            try {
                ...
                val initResult = renderer.initialize(config)
                if (initResult is io.materia.core.Result.Success) {
                    console.log("[Materia] Selected backend: WebGL 2.0 (fallback)")
                    return io.materia.core.Result.Success(renderer)
                } else if (initResult is io.materia.core.Result.Error) {
                    return io.materia.core.Result.Error(
                        initResult.message,
    ...
    private fun createWebGLRenderer(canvas: HTMLCanvasElement, config: RendererConfig): Renderer =
        io.materia.renderer.webgl.WebGLRenderer(canvas)
}
