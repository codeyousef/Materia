...
class ContextLossRecovery {
     ...
     */
    suspend fun recover(device: GpuDevice): io.materia.core.Result<RecoveryStats> {
        if (!isRecovering) {
            return io.materia.core.Result.Success(RecoveryStats(0, 0, 0, 0))
        }
        ...
        val rawDevice = device.unwrapHandle() as? GPUDevice ?: return io.materia.core.Result.Error(
        ...
        if (failures > 0) {
            ...
            return io.materia.core.Result.Success(stats)
        }
    }
