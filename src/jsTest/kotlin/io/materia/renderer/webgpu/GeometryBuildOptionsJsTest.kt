...
class GeometryBuildOptionsJsTest {
    ...
    fun instancingDerivesFromGeometry() {
        ...
        assertTrue(
            options.includeInstancing,
            "Instancing should be enabled when geometry is instanced"
        )
    }
    ...
    fun morphTargetsEnableMorphOption() {
        ...
        val options = descriptor.buildGeometryOptions(geometry)
        assertTrue(
            options.includeMorphTargets,
            "Morph targets should be enabled when geometry defines them"
        )
    }
