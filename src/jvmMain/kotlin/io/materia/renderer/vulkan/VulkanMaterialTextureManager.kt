...
import io.materia.renderer.feature020.OutOfMemoryException
...
internal class VulkanMaterialTextureManager(
    ...
    private val fallbackRoughness =
        createFallbackTexture(Texture2D.solidColor(Color.WHITE).apply { needsUpdate = false })
    private val fallbackMetalness =
    ...
    fun prepare(
        ...
        val requiresNormal =
            descriptor.bindings.any { it.source == MaterialBindingSource.NORMAL_MAP && it.type == MaterialBindingType.TEXTURE_2D }
    ...
    private fun toVulkanAddressMode(wrap: io.materia.renderer.TextureWrap): Int = when (wrap) {
        io.materia.renderer.TextureWrap.REPEAT -> VK_SAMPLER_ADDRESS_MODE_REPEAT
        io.materia.renderer.TextureWrap.MIRRORED_REPEAT -> VK_SAMPLER_ADDRESS_MODE_MIRRORED_REPEAT
