...
import io.materia.renderer.feature020.BufferHandle
import io.materia.renderer.feature020.Color
import io.materia.renderer.feature020.FramebufferHandle
import io.materia.renderer.feature020.PipelineHandle
...
import io.materia.renderer.gpu.GpuContext
...
import io.materia.renderer.gpu.unwrapInstance
import io.materia.renderer.gpu.unwrapPhysicalHandle
...
import io.materia.renderer.webgpu.ColorWriteMask
import io.materia.renderer.webgpu.CullMode
import io.materia.renderer.webgpu.FrontFace
import io.materia.renderer.webgpu.PrimitiveTopology
import io.materia.renderer.webgpu.TextureFormat
import io.materia.renderer.webgpu.VertexAttribute
...
import org.lwjgl.vulkan.VK12.VK_ATTACHMENT_STORE_OP_STORE
import org.lwjgl.vulkan.VK12.VK_BUFFER_USAGE_TRANSFER_DST_BIT
...
import org.lwjgl.vulkan.VK12.VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT
...
import org.lwjgl.vulkan.VK12.VK_MAX_PHYSICAL_DEVICE_NAME_SIZE
...
import org.lwjgl.vulkan.VK12.VK_PHYSICAL_DEVICE_TYPE_DISCRETE_GPU
import org.lwjgl.vulkan.VK12.VK_PIPELINE_BIND_POINT_GRAPHICS
import org.lwjgl.vulkan.VK12.VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT
import org.lwjgl.vulkan.VK12.VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT
import org.lwjgl.vulkan.VK12.VK_PIPELINE_STAGE_TRANSFER_BIT
import org.lwjgl.vulkan.VK12.VK_QUEUE_FAMILY_IGNORED
import org.lwjgl.vulkan.VK12.VK_SAMPLE_COUNT_16_BIT
import org.lwjgl.vulkan.VK12.VK_SAMPLE_COUNT_1_BIT
import org.lwjgl.vulkan.VK12.VK_SAMPLE_COUNT_2_BIT
import org.lwjgl.vulkan.VK12.VK_SAMPLE_COUNT_32_BIT
import org.lwjgl.vulkan.VK12.VK_SAMPLE_COUNT_4_BIT
import org.lwjgl.vulkan.VK12.VK_SAMPLE_COUNT_64_BIT
import org.lwjgl.vulkan.VK12.VK_SAMPLE_COUNT_8_BIT
import org.lwjgl.vulkan.VK12.VK_SHADER_STAGE_FRAGMENT_BIT
import org.lwjgl.vulkan.VK12.VK_SHADER_STAGE_VERTEX_BIT
import org.lwjgl.vulkan.VK12.VK_SHARING_MODE_EXCLUSIVE
import org.lwjgl.vulkan.VK12.VK_STRUCTURE_TYPE_APPLICATION_INFO
...
import org.lwjgl.vulkan.VK12.VK_STRUCTURE_TYPE_COMMAND_POOL_CREATE_INFO
...
import org.lwjgl.vulkan.VK12.VK_STRUCTURE_TYPE_DEVICE_CREATE_INFO
import org.lwjgl.vulkan.VK12.VK_STRUCTURE_TYPE_DEVICE_QUEUE_CREATE_INFO
import org.lwjgl.vulkan.VK12.VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER
import org.lwjgl.vulkan.VK12.VK_STRUCTURE_TYPE_INSTANCE_CREATE_INFO
import org.lwjgl.vulkan.VK12.VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO
import org.lwjgl.vulkan.VK12.VK_STRUCTURE_TYPE_RENDER_PASS_CREATE_INFO
...
import org.lwjgl.vulkan.VK12.VK_VERSION_MAJOR
import org.lwjgl.vulkan.VK12.VK_VERSION_MINOR
import org.lwjgl.vulkan.VK12.VK_VERSION_PATCH
...
import org.lwjgl.vulkan.VK12.vkCreateDevice
import org.lwjgl.vulkan.VK12.vkCreateInstance
import org.lwjgl.vulkan.VK12.vkCreateRenderPass
import org.lwjgl.vulkan.VK12.vkDestroyBuffer
import org.lwjgl.vulkan.VK12.vkDestroyCommandPool
...
import org.lwjgl.vulkan.VK12.vkEnumerateDeviceExtensionProperties
import org.lwjgl.vulkan.VK12.vkEnumeratePhysicalDevices
...
import org.lwjgl.vulkan.VK12.vkResetCommandBuffer
...
class VulkanRenderer(
    ...
    private val pipelineCache =
        object : LinkedHashMap<PipelineCacheKey, VulkanPipeline>(16, 0.75f, true) {
        ...
        return try {
            ...
            if (commandPool == VK_NULL_HANDLE) {
                return io.materia.core.Result.Error(
                    ...
                    io.materia.renderer.RendererInitializationException.DeviceCreationFailedException(
                        BackendType.VULKAN,
    ...
    override fun render(scene: Scene, camera: Camera) {
        ...
        val sceneEnvironmentBinding =
            environmentMgr?.prepare(scene.environment, sceneEnvironmentBrdf)
        ...
        scene.traverseVisible { node ->
            if (node is Mesh && node.visible) {
                ...
                val environmentBindingForMesh = if (requiresEnvironment) {
                    ...
                    environmentMgr?.prepare(cubeTexture, sceneEnvironmentBrdf)
                        ?: return@traverseVisible
                } else {
        ...
        val frameTime = measureTimeMillis {
            ...
            MemoryStack.stackPush().use { stack ->
                ...
                for (drawInfo in drawInfos) {
                    ...
                    val environmentFallback =
                        environmentBinding?.usingFallbackEnvironment == true ||
                    ...
                    val morphInfluenceSource: List<Float>? = drawInfo.mesh.morphTargetInfluences
                        ?: (drawInfo.mesh.userData["morphTargetInfluences"] as? List<*>)
                    ...
                    val descriptorSetsBuffer =
                        stack.mallocLong(1 + (if (hasMaterialSet) 1 else 0) + (if (hasEnvironmentSet) 1 else 0))
                    descriptorSetsBuffer.put(0, descriptorHandle)
                    var descriptorIndex = 1
                    if (hasMaterialSet) {
                        descriptorSetsBuffer.put(
                            descriptorIndex,
                            drawInfo.textureBinding.descriptorSet
                        )
                        descriptorIndex += 1
                    }
                    if (hasEnvironmentSet) {
                        descriptorSetsBuffer.put(
                            descriptorIndex,
                            environmentBinding!!.descriptorSet
                        )
                        descriptorIndex += 1
                    }
    ...
    private fun createDescriptorResources() {
        ...
        if (descriptorSetLayout == VK_NULL_HANDLE) {
            MemoryStack.stackPush().use { stack ->
                ...
                val layoutResult =
                    vkCreateDescriptorSetLayout(deviceHandle, layoutInfo, null, pLayout)
    ...
    private fun createMaterialTextureResources() {
        ...
        if (materialTextureDescriptorPool == VK_NULL_HANDLE && layoutBindings.isNotEmpty()) {
            MemoryStack.stackPush().use { stack ->
                val sampledImageCount =
                    layoutBindings.count { it.type != MaterialBindingType.SAMPLER }
                val samplerCount = layoutBindings.count { it.type == MaterialBindingType.SAMPLER }
                val poolSizeCount =
                    (if (sampledImageCount > 0) 1 else 0) + (if (samplerCount > 0) 1 else 0)
    ...
    private fun ensureMeshBuffers(mesh: Mesh, descriptor: MaterialDescriptor): VulkanMeshBuffers? {
        ...
        val instanceCount =
            if (geometryBuffer.instanceCount > 0) geometryBuffer.instanceCount else 1
    ...
    private fun createInstance(enableValidation: Boolean): VkInstance? {
        return MemoryStack.stackPush().use { stack ->
            ...
            val appInfo = VkApplicationInfo.calloc(stack)
                ...
                .pApplicationName(stack.UTF8("Materia Application"))
    ...
    private fun createLogicalDevice(
        ...
        return MemoryStack.stackPush().use { stack ->
            ...
            extensions.put(
                0,
                stack.UTF8(org.lwjgl.vulkan.KHRSwapchain.VK_KHR_SWAPCHAIN_EXTENSION_NAME)
            )
    ...
    private fun enumerateDeviceExtensions(
        device: VkPhysicalDevice,
        stack: MemoryStack
    ): Set<String> {
        val countBuffer = stack.mallocInt(1)
        val initialResult =
            vkEnumerateDeviceExtensionProperties(device, null as ByteBuffer?, countBuffer, null)
    ...
    private fun getPhysicalDeviceInfo(physicalDevice: VkPhysicalDevice): String {
        return MemoryStack.stackPush().use { stack ->
            ...
            "$deviceName (Vulkan ${VK_VERSION_MAJOR(properties.apiVersion())}.${
                VK_VERSION_MINOR(
    ...
    private fun BindingPair.textureSample(coord: String): String =
        "texture(${samplerRef()}, $coord)"
    ...
    private fun composeVertexShader(
        ...
        return buildString {
            ...
            if (features.usesInstancing) {
                ...
                val columns = (0 until 4).map { index ->
                    instanceAttributeNames.getOrNull(index) ?: identityColumns[index]
                }
    ...
    private fun composeFragmentShader(
        ...
        val aoTerm =
            if (features.usesAmbientOcclusionMap && aoSampleExpr != null) aoSampleExpr else "1.0"
    ...
    private fun buildShaderProgramConfig(
        ...
        val prefilterPair =
            bindingPair(environmentBindingLookup, MaterialBindingSource.ENVIRONMENT_PREFILTER)
        ...
        val usesMorphNormals =
            metadata.bindings.any { it.attribute == GeometryAttribute.MORPH_NORMAL }
