...
import io.kreekt.camera.StereoCamera
import io.kreekt.camera.StereoUtils
...
import io.kreekt.renderer.createTestCube
...
class HeadsetEnvironmentRegressionTest {
    ...
    fun headsetSceneRendersOnBothPrimaryBackends() = runBlocking {
        ...
        val stereoCamera = StereoCamera().apply {
            ...
            convergence = StereoUtils.calculateOptimalConvergence(nearObjects = 0.5f, farObjects = 5f)
            update(headsetRig)
        }
        ...
        assertTrue(result is IBLResult.Success<*>, "Environment processing must succeed for headset regression")
        val maps = (result as IBLResult.Success<*>).data as IBLEnvironmentMaps
        assertSame(maps.prefilter, scene.environment, "Scene should consume prefiltered environment cubemap")
        assertSame(maps.brdfLut, scene.environmentBrdfLut, "Scene should consume generated BRDF LUT")
        ...
        assertEquals(config.prefilterSize, maps.prefilter.size, "Prefilter cube needs expected dimension")
        assertEquals(config.brdfLutSize, maps.brdfLut.width, "BRDF LUT width should match configuration")
        assertEquals(config.brdfLutSize, maps.brdfLut.height, "BRDF LUT height should match configuration")
        ...
        renderers.forEach { renderer ->
            ...
            assertTrue(init is Result.Success<*>, "Renderer init should succeed for ${renderer.backend}")
